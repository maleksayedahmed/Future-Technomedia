                ...
                <div class="content">
                    ...
                    <div class="fsp-filter">
                        ...
                        <div class="gallery-filters">
                            <a href="#" class="gallery-filter {{ !isset($selectedCategory) ? 'gallery-filter-active' : '' }}" data-filter="*">All ({{ $projects->count() }})</a>
                            @foreach($categories as $category)
                                @if($category->projects_count > 0)
                                    <a href="#" class="gallery-filter {{ isset($selectedCategory) && $selectedCategory == $category->id ? 'gallery-filter-active' : '' }}" data-filter=".category-{{ $category->id }}">
                                            ...
                                            <div class=" fl-wrap full-height">
                                                <div class="hd-box-wrap">
                                                    <h2>
                                                        ...
                                                        @else
                                                            <a href="portfolio-single.html">{{ $project->title }}</a>
                                                        @endif
                                                    </h2>
                                                    <p>
                                                        @if($project->category)
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        @if(isset($selectedCategory))
            ...
            const filterButton = document.querySelector('.gallery-filter[data-filter=".category-{{ $selectedCategory }}"]');
            if (filterButton) {
