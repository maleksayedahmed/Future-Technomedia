                                ...
                                @foreach ($settings as $setting)
                                    <div class="col-md-6 mb-3">
                                        ...
                                        @if ($setting->type === 'textarea')
                                            <textarea name="settings[{{ $setting->key }}]" id="setting_{{ $setting->key }}" class="form-control" rows="4">{{ $setting->value }}</textarea>
                                        @elseif($setting->type === 'boolean')
                                            <div class="form-check">
                                                <input type="hidden" name="settings[{{ $setting->key }}]" value="0">
                                                <input type="checkbox" name="settings[{{ $setting->key }}]"
                                                    id="setting_{{ $setting->key }}" class="form-check-input"
                                        ...
                                        @elseif($setting->type === 'image')
                                            <div>
                                                ...
                                                <input type="file" name="{{ $setting->key }}"
                                                    id="setting_{{ $setting->key }}" class="form-control" accept="image/*">
                                            </div>
                                        @else
                                            <input type="{{ $setting->type === 'url' ? 'url' : 'text' }}"
                                                name="settings[{{ $setting->key }}]" id="setting_{{ $setting->key }}"
